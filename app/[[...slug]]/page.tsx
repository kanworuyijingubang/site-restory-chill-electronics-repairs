...
import { absoluteUrl, normalizePath, pageByPath, pages, SITE_NAME, SITE_URL, pageUpdatedDate, pageCheckedDate, formatPageDate } from "@/lib/site-data";
...
export default async function Page({ params }: { params: Promise<{ slug?: string[] }> }) {
  ...
  })).filter((crumb) => pageByPath.has(crumb.item.slice(SITE_URL.length)))
    .map((crumb, index) => ({ ...crumb, position: index + 2 }));
  const graph: Record<string, unknown>[] = [
    { "@type": "WebPage", "@id": absoluteUrl(page.path), url: absoluteUrl(page.path), name: page.title, description: page.description, dateModified: pageUpdatedDate(page), isPartOf: { "@id": `${SITE_URL}/#website` } },
  ...
  if (page.path.startsWith("/guides/") || page.path === "/updates/" || page.path === "/achievements/" || page.path.startsWith("/story/")) graph.push({ "@type": "Article", headline: page.title, description: page.description, datePublished: "2026-08-13", dateModified: pageUpdatedDate(page), author: { "@id": `${SITE_URL}/#organization` }, publisher: { "@id": `${SITE_URL}/#organization` }, mainEntityOfPage: absoluteUrl(page.path) });

  return <><ContentRenderer page={page} /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@graph": graph }).replace(/</g, "\\u003c") }} /><span className="sr-only">Content checked {formatPageDate(pageCheckedDate(page))}</span></>;
}
